...
function validateResource ({
  ...
  if (invalidProperty) {
    throw new Errors.InvalidValue(`invalid property value ${invalidProperty.value} for property ${invalidProperty.path}`)
  }
  ...
  if (!partial && model.subClassOf !== 'tradle.Enum') {
    required.forEach(propertyName => {
      if (!(propertyName in resource)) {
        ...
        throwPropertyError(propertyName, `"${title}" is required`, Errors.Required)
      }
    })
